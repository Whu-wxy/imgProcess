...
Mat screen0(Mat src)
{
    ...
    Mat img(src.cols*12, src.rows*12, CV_8UC1, Scalar(0, 0, 0));
    Mat kernel = (Mat_<char>(12, 12) << 144,140,132,122,107,63,54,93,106,123,133,142,
    ...
    for(int i=0; i<img.rows; i++)
    {
        ...
        {
            int l = j % 12;
            int pixelValue = ceil(src.at<char>(i/12, j/12) * 255.0/144);
            if(pixelValue > kernel.at<char>(k*12+l))
                img.at<char>(i, j) = 0;
            else
                img.at<char>(i, j) = 255;
        }
